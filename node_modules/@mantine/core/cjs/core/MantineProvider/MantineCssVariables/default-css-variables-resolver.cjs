...
const require_get_css_color_variables = require("./get-css-color-variables.cjs");
//#region packages/@mantine/core/src/core/MantineProvider/MantineCssVariables/default-css-variables-resolver.ts
...
const defaultCssVariablesResolver = (theme) => {
	...
	require_keys.keys(theme.colors).forEach((color) => {
		...
		if (require_virtual_color.isVirtualColor(value)) {
			Object.assign(result.light, require_get_css_color_variables.getCSSColorVariables({
				theme,
				name: value.name,
				color: value.light,
				colorScheme: "light",
			...
			Object.assign(result.dark, require_get_css_color_variables.getCSSColorVariables({
				theme,
				name: value.name,
				color: value.dark,
				colorScheme: "dark",
			...
			result.light[`--mantine-color-${value.name}-contrast`] = require_get_contrast_color.getVirtualColorContrast(value, theme, "light");
			result.dark[`--mantine-color-${value.name}-contrast`] = require_get_contrast_color.getVirtualColorContrast(value, theme, "dark");
			return;
		}
